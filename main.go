...
func demultiplexingFunc(dataSourceChan chan int, amount int) ([]chan int, <-chan struct{}) {
	...
	//Эту переменную будет использовать для идентификации необходимости закрыть канал
	...
	go func() {
		...
		//После посылки сообщение отправляем сигнал каналу для закрытия (закрываем канал done)
		close(done)
	}()
...
func multiplexingFunc(done <-chan struct{}, channels ...chan int) <-chan int {
	...
	multiplex := func(c <-chan int) {
		...
		for {
			select {
			//если поступило сообщение, отправляем его в мультиплексор
			case v := <-c:
				multiplexedChan <- v
				//Если получили сигнал о необходимости закрыть канал, выходим из бесконечного цикла
			case <-done:
				return
			}
		}
...
func main() {
	...
	var consumers, done = demultiplexingFunc(startDataSource(), 5)
